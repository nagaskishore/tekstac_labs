...
# =============================================================================
# PHASE 4: LANGGRAPH STATE MANAGEMENT MODELS
# =============================================================================

class WorkflowState(BaseModel):
    """
    LangGraph workflow state persistence model
    Tracks complete workflow state for Phase 4 implementation
    """
    # Workflow metadata
    thread_id: Optional[str] = None
    current_node: str = "start"
    workflow_status: Literal["initialized", "collecting_info", "planning", "optimizing", "awaiting_approval", "completed", "failed"] = "initialized"
    ...
    def mark_error(self, error_msg: str, node: str):
        """Record an error in the workflow"""
        self.error_count += 1
        self.last_error = error_msg
        self.tool_errors.append(f"{node}: {error_msg}")
    
    def mark_fallback(self, original_tool: str, node: str):
        """Record fallback tool usage"""
        self.fallback_used.append(f"{node}: {original_tool} -> web_search_service")
    
    def update_timing(self, node: str, duration: float):
        """Track node execution time"""
        self.node_timings[node] = duration


class CheckpointData(BaseModel):
    """
    Human-in-the-loop state snapshots for approval workflows
    Enables state restoration after user feedback
    """
    checkpoint_id: str
    thread_id: str
    ...
    # Checkpoint metadata
    checkpoint_type: Literal["approval", "clarification", "error_recovery"] = "approval"
    node_before_checkpoint: str
    next_node: str
    ...
    def mark_responded(self, response: str):
        """Mark checkpoint as responded to by user"""
